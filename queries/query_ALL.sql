...
select top 100 ca_zip, sum(ws_sales_price)
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('chartreuse','firebrick','khaki','misty','spring','blue') and
...
select top 100 
 ...
 where
  ...
  ca_county in ('Fond du Lac County','Taylor County','De Witt County','Heard County','Scurry County') and
...
select top 100 i_item_id
 ...
 and i_manufact_id in (954,786,761,751)
...
from (select avg(ss_list_price) B1_LP
      ...
      where ss_quantity between 0 and 5
        and (ss_list_price between 129 and 129+10 
             or ss_coupon_amt between 12475 and 12475+1000
     ...
     (select avg(ss_list_price) B5_LP
      ...
      where ss_quantity between 21 and 25
        and (ss_list_price between 51 and 51+10
          or ss_coupon_amt between 5959 and 5959+1000
     ...
     (select avg(ss_list_price) B6_LP
      ...
      where ss_quantity between 26 and 30
        and (ss_list_price between 55 and 55+10
          or ss_coupon_amt between 2001 and 2001+1000
...
select top 100  
 ...
 from (
    (select 
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 53260 and 53260+28800 
...
select top 100 case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 0.001*219087
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 0.001*121786
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 0.001*28305
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 0.001*62052
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 0.001*114122
            then (select avg(ss_ext_sales_price)
                  from store_sales
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Jewelry','Electronics','Music') and
          i_class in ('mens watch','wireless','classical')
         )
      or (i_category in ('Sports','Children','Books') and
          i_class in ('baseball','infants','reference') 
        ))
...
select top 100 c_customer_id as customer_id
 ...
 where ca_city	        =  'Mount Olive'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  52066
   and ib_upper_bound   <=  52066 + 50000
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('chartreuse','cornsilk','midnight'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 1998
 and     d_moy                   = 7
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ss_net_profit between 100 and 200  
     ...
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('GA', 'MO', 'IL')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('MS', 'GA', 'MN')
